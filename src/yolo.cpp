...
cv::Mat Yolo::drawPred(Mat& img, vector<Output> result, vector<Scalar> color) {
    for (size_t i = 0; i < result.size(); i++) {
        ...
        //        className[result[i].id] != "car") continue;
        ...
        //        }
        //        // LOG_F(INFO, "%d,  %d,  %d,  %d", left +
        //        int(result[i].box.width * 0.5), top,
        //        //       logo.cols, logo.rows);
        //        Rect rect = Rect(0, 0, img.cols, img.rows);
        //        if (isInside(Rect(left + int(result[i].box.width * 0.5), top -
        //        logo.rows, logo.cols, logo.rows), rect)) {
        //            imageROI = img(Rect(left + int(result[i].box.width * 0.5),
        //            top - logo.rows, logo.cols, logo.rows)); resize(logo,
        //            logo, imageROI.size()); addWeighted(imageROI, 0.5, logo,
        //            1, 0, imageROI);
        //        }
    }
    return img;
