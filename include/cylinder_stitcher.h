...
struct Cylinder {
    ...
    Cylinder(Eigen::Matrix<float, 3, 1> _center, float _r, Eigen::Matrix<float, 3, 3> _rotation) : center(_center),
                                                                                                   r(_r), rotation(
                    _rotation) {}
};
...
struct Ray {
    ...
    Ray(Eigen::Matrix<float, 3, 1> _origin, Eigen::Matrix<float, 3, 1> _dir) : origin(_origin), dir(_dir) {
        this->dir.normalize();
    }
...
struct PinholeCamera {
    ...
    PinholeCamera(Eigen::Matrix3f _K, Eigen::Matrix3f _R, Eigen::Matrix<float, 3, 1> _T, bool c2w = false) : K(_K) {
        if (c2w == false) {
            this->R = _R;
            this->T = _T;
...
struct View {
    ...
    View(cv::Mat _image, cv::Mat _mask, PinholeCamera _camera) : image(_image), mask(_mask), camera(_camera) {}
};
...
struct CylinderImage {
    ...
    CylinderImage(cv::Mat _image, cv::Mat _mask, int2 _min_uv) : image(_image), mask(_mask), min_uv(_min_uv) {}
};
...
//  Cylinder calCylinder(std::vector<PinholeCamera> cameras) {}
    std::vector<std::vector<float4> > findCorrespondences(std::vector<View> views);

    CylinderImage projToCylinderImage(std::vector<View> views, Cylinder cylinder);

    void alignImagesCPU(std::vector<CylinderImage> &cylinder_images, std::vector<std::vector<float4> > match_corrs);

    void alignImagesGPU(std::vector<CylinderImage> &cylinder_images, std::vector<std::vector<float4> > match_corrs);

//  std::vector<cv::Mat> projToScreen(std::vector<CylinderImage> cylinder_images, std::vector<PinholeCamera> cameras) {}
    void show(CylinderImage cyl_image, std::vector<View> views, Cylinder cylinder);
...
private:
    ...
    Cylinder cylinder_;
    double theta_step_, phi_step_, global_min_theta_, global_max_theta_, global_min_phi_, global_max_phi_;
};
